...
class KeyWord:
    ...
    def key_goto(self, step):
        self.webdriver.get(step.参数)

    def key_touch(self,step):
        ele = self.find_element(step.元素路径)
    ...
    def key_input(self, step):
        ...
        ele.clear()
        ele.send_keys(step.参数)

    # def key_verify(self, step,locator, verify_name, expression, value, *args):
...
class Runner:
    ...
    def run(self, test: unittest.TestCase):
        ...
        for index, step in enumerate(self.case.steps, start=1):
            ...
            print(f"{index}. {step.步骤序号} {step.步骤名} ... ... ", end="")
            try:
                step_func(step)
                print("OK <br />")
